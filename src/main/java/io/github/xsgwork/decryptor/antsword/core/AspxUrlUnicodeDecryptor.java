...
public class AspxUrlUnicodeDecryptor extends AbstractNormalParamDecryptor {
    ...
    protected Map<String, String> convertPasswordParam(Map<String, String> paramMap, String password) {
        ...
        // unicode解码
        String unicodeDecodedData = DecodeUtil.unicodeDecode(paramValue);
        // 尝试提取bd函数中的内容
        String extractedData = StringExtractUtil.extractContent(unicodeDecodedData, "System.Convert.FromBase64String(\"", "\")),\"unsafe\");");
        if (StrUtil.isBlank(extractedData)) {
            result.put(password, unicodeDecodedData);
        } else {
